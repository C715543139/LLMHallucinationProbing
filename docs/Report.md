...
Phase 4 复测进一步发现，在 Linux + RTX 3090 环境下，`eager + float16` 仍会产生 NaN，真正稳定的运行路径是 `eager + bfloat16`。在该设置下，全量 hidden-only baseline 达到 Accuracy 0.8082、Macro-F1 0.8081、AUROC 0.8897；在 600/150/150 子集上，`Hidden + top-16 head attention score` 融合取得最佳 Accuracy / F1（0.8867 / 0.8865），`Hidden + top-head + attention output` 三路融合取得最佳 AUROC（0.9403）。这些结果说明 attention score 对 hidden state 具有稳定互补性，而 attention output 更适合作为排序补充信号。本报告据此总结 Phase 1 至 Phase 4 的实现、核心方法、实验结果与工程结论。
...
当前 head selection 的验证集 AUROC 分布落在 0.6069 至 0.6776 之间，最强单 head 为 `L16-H05`。此外，A9 的错误修正矩阵显示虽然有 3 个样本的融合路由发生变化，但净纠错数为 0，说明当前简单 gated fusion 还不足以稳定替代直接特征拼接。
...
Phase 4 的结果表明，attention 特征并非简单的“附属解释变量”，而是真正可以补充 hidden baseline 的结构化信号。其原因在于：hidden state 更偏向整句压缩表示，而 attention score 显式保留了 `last token` 与 `subject / relation / tail` 之间的对齐关系；在对长度因素做残差化处理、并通过验证集筛选出高价值 head 之后，这类结构信号更容易与 hidden 表征形成互补。A6 优于 A5，也说明“先筛 head 再融合”比“保留全部 score 统计再拼接”更有效，因为它减少了噪声维度并保留了最具判别力的注意力模式。

### 6.7 Phase 4 暴露的工程边界

Phase 4 同时揭示了两个重要边界。第一，Linux 平台本身不足以自动解决数值问题；真正决定实验是否可用的是 `dtype + attention implementation` 的组合，当前可复现实验必须走 `eager + bfloat16`。第二，attention output 虽然能改善 AUROC，但并未像 top-head score 那样稳定提升准确率，这提示 attention 模块写回 residual stream 的激活更适合作为排序补充而非主决策特征。再加上 gated fusion 在当前设置下没有带来净纠错，说明后续改进应优先集中在更稳健的特征筛选、路由策略与全量数据验证，而不是盲目增加融合复杂度。
...
5. **复杂融合策略尚未形成稳定优势**：当前 gated fusion 没有带来净纠错，说明简单路由机制不足以替代直接特征融合。
...
当前结果表明，PPL 虽然具备一定排序能力，但最终分类性能明显不足；相比之下，基于隐藏状态的 SAPLMA 能够更有效地利用模型内部表征。Phase 3 显示真实性信号在中后层更强，`layer 17 + last + logistic` 明显优于固定最后层的基线。Phase 4 则进一步表明：在解决 NaN 问题后，attention score 已具备独立判别力；通过 validation-based head selection 后与 hidden state 融合，可以把子集 Accuracy 从 0.8667 提升到 0.8867；而加入 attention output 后可以把 AUROC 提升到 0.9403。
...
3. 已生成：layer-head AUROC heatmap、特征差异 boxplot、方法 Accuracy / AUROC 对比图、correction matrix
4. 可补充：PPL 分数分布直方图（真/假对比）
