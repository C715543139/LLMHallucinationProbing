...
Phase 4 选择了项目计划中的方向 A：基于注意力模式的幻觉检测。当前实现以 Phase 3 中验证表现较好的 `layer 17 + last + logistic` 隐藏状态配置作为主基线，并进一步提取 20 维注意力统计特征，比较以下三种变体：

1. **attention-only**：只使用注意力统计特征；
2. **hidden-only**：只使用 `layer 17 + last` 隐藏状态；
3. **hidden+attention**：将隐藏状态与注意力特征直接拼接后输入逻辑回归分类器。

根据 `experiments/results/advanced/attention_ablation_logistic_layer17_last.json`，三种变体的测试集结果如下：
...
以验证集 Accuracy 作为模型选择标准时，最佳变体为 **hidden+attention**。这一结果表明，当前构造的注意力统计特征本身并不足以直接承担真假分类任务，但作为辅助特征与隐藏状态融合后，能够带来小幅而稳定的 Accuracy 和 Macro-F1 提升。与同一实验中的 hidden-only 变体相比，hidden+attention 的测试集 Accuracy 提升约 **0.0063**，Macro-F1 提升约 **0.0064**；AUROC 基本持平，说明注意力特征带来的收益主要体现在固定决策边界附近的分类修正，而不是显著改变整体排序能力。

进一步地，结果文件中还对真 / 假样本的注意力特征均值差异进行了汇总。在测试集上，当前差异幅度最大的特征包括：
...
其中 `sequence_length` 的均值差异最大（真样本平均长度 11.2681，假样本平均长度 11.5382），这提示当前注意力特征仍然混入了一部分表面统计信息。与此同时，`subject_attn_ratio`、`subject_attn_last_layer` 和 `cross_head_agreement_mean` 在真样本上整体更高，说明真实陈述在当前模型中确实表现出更集中、更一致的主语相关注意力模式。对应图像已保存为：
...
### 6.6 为什么注意力特征单独较弱、融合后略有增益

Phase 4 的结果呈现出一个较有代表性的模式：attention-only 几乎退化到随机水平，而 hidden+attention 相比 hidden-only 则有小幅提升。这说明当前实现中的注意力统计特征并没有形成足够强的独立判别空间，但它们仍然为隐藏状态分类器提供了补充信息。

造成 attention-only 较弱的原因可能包括：

- 当前采用的是陈述句而非问答格式，注意力图中缺少明确的 query-answer 对齐结构；
- `subject / relation / tail` 锚点目前主要通过轻量规则抽取，仍存在定位误差；
- 注意力统计特征维度较低，难以完整保留多层多头结构中的细粒度差异；
- 当前特征中仍混入了 `sequence_length`、token 数量等表面因素。

另一方面，fusion 略优于 hidden-only，说明注意力虽然不足以独立完成判别，但在部分边界样本上确实提供了额外线索。当前结果更接近“注意力作为辅助读出信号”而不是“注意力取代隐藏状态成为主特征”。这也与当前数值观察一致：fusion 主要提升 Accuracy / Macro-F1，而 AUROC 与 hidden-only 几乎相同。

### 6.7 当前 Phase 4 的主要经验与限制

当前 Phase 4 的一个重要经验是：注意力特征工程不能只追求“能提出来”，还需要持续控制表面统计特征的污染。当前测试集上差异最大的特征是 `sequence_length`，这说明即便注意力矩阵来自模型内部状态，其统计摘要也可能被句长等外在因素牵引。后续若要进一步提升 Phase 4 效果，至少需要沿以下方向改进：

- 对注意力特征做长度归一化或残差化处理；
- 使用更稳健的主语/关系词对齐方法；
- 引入逐层或逐头的选择机制，而不是直接做全局均值统计；
- 尝试更适合融合异构特征的分类器与显式消融策略。
...
进一步地，Phase 3 显示真实性相关信号在中后层最强，而最后层并非最优读出位置；在最后层内部，`last` token 表示又明显优于 `mean` 与 `first` pooling。当前 Phase 4 表明，注意力统计特征单独使用时几乎无法稳定完成真假检测，但与 `layer 17 + last` 隐藏状态融合后，可以将测试集 Accuracy 从 0.8003 提升到 0.8067，Macro-F1 从 0.8001 提升到 0.8066。这说明在该任务上，隐藏状态仍是主导性表征，而注意力模式更适合作为辅助信号。下一阶段将围绕注意力特征去偏、FFN 特征实现以及更广泛泛化验证继续推进。
