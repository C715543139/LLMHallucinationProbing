...
当前已完成 Phase 1 至 Phase 4（方向 A：注意力模式）。Phase 2 基线结果显示，PPL 在测试集上的 Accuracy 为 0.5293、Macro-F1 为 0.4180、AUROC 为 0.6784；在固定最后层与 `last` token 设置下，重跑后的 SAPLMA-MLP 测试集平均 Accuracy 为 0.7697、Macro-F1 为 0.7696、AUROC 为 0.8676。Phase 3 进一步表明，真实性相关信号并不在最后层最强：对 28 个 Transformer block 的 `last` token 做逐层分析后，验证集最优层为 layer 17，其测试集 Accuracy 为 0.7987、Macro-F1 为 0.7986、AUROC 为 0.8878；在最后层的 token 表示比较中，`last` pooling 优于 `mean` pooling 和 `first` pooling，测试集 Accuracy 分别为 0.7433、0.7021 和 0.3867。Phase 4 首轮逻辑回归融合实验仅取得小幅收益（hidden+attention Accuracy 0.8067），但在完成“去除表面统计特征 + 限定高性能层 13-20 + 加入模型自发现 key token 特征 + 使用 MLP 融合”两轮迭代后，改进版 hidden+attention 在测试集上提升到 **0.8357 ± 0.0051**，Macro-F1 提升到 **0.8357 ± 0.0051**，AUROC 提升到 **0.9217 ± 0.0068**。

这些结果说明，相比单纯依赖输出概率，模型隐藏状态以及“取哪一层、取哪个 token 表示”对真假检测性能具有更直接的影响；注意力统计特征在单独使用时仍不足以承担主判别责任，但在完成去污染、层选择、key token 锚点和非线性融合后，已能为隐藏状态提供更稳定的补充增益。本报告基于当前工作区中的最终结果文件，同步总结 Phase 1 至 Phase 4 的实现、实验结果与工程状态。

**关键词**：幻觉检测；大语言模型；隐藏状态；注意力模式；PPL；SAPLMA；Qwen2-1.5B
...
本项目采用第二类思路，以论文 _The Internal State of an LLM Knows When It's Lying_ 为主要参考，围绕 True-False Dataset 中的陈述句开展实验。项目首先实现两类基线方法：其一是基于序列困惑度的 PPL 方法，其二是基于隐藏状态分类的 SAPLMA 方法。随后，项目继续分析不同层深度、不同 token 位置表示对真实性检测的影响，并在当前阶段实现了 Phase 4 的方向 A：从注意力矩阵中构造统计特征，探索其单独使用与和隐藏状态融合时的检测效果。

本报告对应项目当前的阶段性实验结果汇总，覆盖已完成的 Phase 1、Phase 2、Phase 3 与 Phase 4（方向 A）。报告既总结当前实现结果，也为后续 FFN 特征与更系统的进阶消融实验保留扩展空间。
...
本项目聚焦于课程要求中的基础任务、分析任务与一项进阶任务，已在统一实验设置下完成以下工作：
...
4. 实现基于注意力模式的统计特征，并比较 attention-only、hidden-only 和 hidden+attention 三类变体。

下一阶段将继续探索 FFN 激活、注意力特征去偏与更丰富的特征融合方式是否能够进一步提升检测性能。
...
- **注意力模式分析**：提取多层多头注意力矩阵，围绕 `subject / relation / tail` 三类锚点构造统计特征，并与隐藏状态基线进行融合比较。

当前 Phase 4 已实现的注意力特征包括：注意力熵、面向主语/关系词/句尾 token 的注意力占比、最后 token 指向锚点的注意力质量、多头一致性，以及锚点 token 数与序列长度等。工程上，当前实现优先使用 tokenizer 的 `offset_mapping` 将字符级规则跨度对齐到 token；若当前 tokenizer 不支持 offset mapping，则退化为基于 token 位置的轻量规则。

对应实现文件：

- 注意力特征提取：`src/features/attention.py`
- 注意力增强方法：`src/methods/advanced.py`
...
- `tests/phase4/`：验证注意力特征提取、attention-only / hidden-only / fusion 消融逻辑、可视化与小规模真实模型集成路径。

同时，结果文件中已统一记录随机种子、阈值选择方式与运行环境元数据。这为当前报告中的 Phase 2 / Phase 3 / Phase 4 结果提供了较高的实现可信度，也为跨设备复现实验提供了依据。
...
### 5.5 Phase 4：基于注意力模式的增强检测结果

Phase 4 选择了项目计划中的方向 A：基于注意力模式的幻觉检测。该阶段经历了两轮可验证的改进迭代：

1. **原始基线**：`layer 17 + last + logistic` 隐藏状态，与 20 维注意力统计特征做早期拼接；
2. **迭代 1（改进特征 + Logistic Regression）**：移除 `sequence_length`、`subject_token_count`、`relation_token_count` 等表面统计特征，仅保留高性能层 13-20 的注意力统计，并对 target mass 做长度归一化；
3. **迭代 2（改进特征 + MLP）**：在迭代 1 基础上加入模型自发现的 key token 注意力特征，并使用 MLP 融合隐藏状态与注意力特征。

三种变体始终保持不变：

1. **attention-only**：只使用注意力统计特征；
2. **hidden-only**：只使用 `layer 17 + last` 隐藏状态；
3. **hidden+attention**：将隐藏状态与注意力特征直接拼接后输入逻辑回归分类器。

原始 Phase 4 逻辑回归基线（`experiments/results/advanced/attention_ablation_logistic_layer17_last.json`）的测试集结果如下：

| 变体 | Feature Dim | Test Accuracy | Test Macro-F1 | Test AUROC |
| ---- | ----------- | ------------- | ------------- | ---------- |
| attention-only | 20 | 0.4976 | 0.4974 | 0.5031 |
| hidden-only | 1536 | 0.8003 | 0.8001 | 0.8879 |
| hidden+attention | 1556 | **0.8067** | **0.8066** | 0.8878 |

以验证集 Accuracy 作为模型选择标准时，原始最佳变体为 **hidden+attention**。这一结果说明注意力统计特征本身并不足以直接承担真假分类任务，但作为辅助特征与隐藏状态融合后，能够带来小幅而稳定的 Accuracy 和 Macro-F1 提升。与同一实验中的 hidden-only 变体相比，hidden+attention 的测试集 Accuracy 提升约 **0.0063**，Macro-F1 提升约 **0.0064**；AUROC 基本持平，说明原始注意力特征带来的收益主要体现在固定决策边界附近的分类修正，而不是显著改变整体排序能力。

在此基础上，改进迭代的实测结果如下：

| 结果文件 | 分类器 | attention-only | hidden-only | hidden+attention |
| -------- | ------ | -------------- | ----------- | ---------------- |
| `attention_ablation_logistic_layer17_last_improved.json` | Logistic | 0.4976 | 0.7987 | **0.8051** |
| `attention_ablation_mlp_layer17_last_improved.json` | MLP | 0.5008 ± 0.0027 | 0.8347 ± 0.0106 | **0.8357 ± 0.0051** |

可以看到，**仅做 P0 级别的线性特征工程并不足以超过原始 0.8067 基线**；真正带来显著收益的是“更干净的注意力特征 + key token 锚点 + 非线性融合”的组合。最终 MLP 版 hidden+attention 相比原始逻辑回归基线提升约 **0.0290 Accuracy**、**0.0291 Macro-F1** 和 **0.0339 AUROC**。

进一步地，改进版结果文件对真 / 假样本的注意力特征均值差异进行了汇总。在测试集上，差异幅度最大的特征已从 `sequence_length` 转移为：
...
这说明经过改进后，注意力特征空间中的主导差异已从句长等表面统计量，转移到了模型自发现的 key token 及主语相关注意力模式上。与此同时，`subject_attn_ratio`、`subject_attn_last_layer`、`key_attn_ratio` 和 `key_attn_last_layer` 在真样本上整体更高，说明真实陈述在当前模型中更容易形成“最后 token 回看关键知识锚点”的集中注意力模式。对应图像已保存为：

- `experiments/results/advanced/attention_ablation_accuracy.png`
- `experiments/results/advanced/attention_feature_deltas.png`
- `experiments/results/advanced/attention_ablation_logistic_layer17_last_improved_accuracy.png`
- `experiments/results/advanced/attention_ablation_mlp_layer17_last_improved_accuracy.png`
...
### 6.6 为什么注意力特征单独较弱、融合后在改进版中明显增强

Phase 4 的结果呈现出一个较有代表性的模式：attention-only 始终接近随机水平，而 hidden+attention 的收益取决于特征工程质量与融合分类器。原始逻辑回归版本仅有小幅提升；在改进版中，attention-only 依旧不能独立承担判别任务，但 hidden+attention 已能把整体 Accuracy 提升到 0.8357。这说明注意力统计特征并没有形成足够强的独立判别空间，但经过去污染和更合理的锚点定义后，它们可以为隐藏状态分类器提供更高质量的补充信息。

造成 attention-only 较弱的原因可能包括：

- 当前采用的是陈述句而非问答格式，注意力图中缺少明确的 query-answer 对齐结构；
- 即使加入了 key token 锚点，注意力特征仍主要扮演辅助信号而非主表示；
- 注意力统计特征维度仍明显低于 1536 维隐藏状态，单独使用时表达能力有限；
- 若继续使用线性分类器，跨模态交互很难被充分建模。

另一方面，改进版 MLP fusion 略优于同配置的 hidden-only（0.8357 vs 0.8347），说明注意力虽然不足以独立完成判别，但在部分边界样本上确实提供了额外线索。当前结果依然更接近“注意力作为辅助读出信号”而不是“注意力取代隐藏状态成为主特征”，但与原始版本相比，其辅助价值已经更稳定、更可测。

### 6.7 当前 Phase 4 的主要经验与限制

当前 Phase 4 的一个重要经验是：注意力特征工程不能只追求“能提出来”，还需要持续控制表面统计特征的污染，并为融合阶段提供足够的非线性建模能力。原始版本中测试集差异最大的特征是 `sequence_length`；改进版中这一现象已消失，top 特征变为 `key_attn_last_layer`、`key_attn_ratio` 和 `last_to_key`，说明特征空间已经更接近真正的注意力模式差异。后续若要进一步提升 Phase 4 效果，至少需要沿以下方向改进：

- 继续探索逐层 / 逐头注意力特征，而不止于层 13-20 的区间汇总；
- 尝试 stacking 或 gated fusion，进一步放大注意力对边界样本的贡献；
- 评估 FFN 激活等方向，验证是否能与当前 improved Phase 4 形成互补；
- 在更多模型与随机种子上复验当前改进结果的稳定性。
...
以下模块或任务仍属于 Phase 4 后续扩展内容或 Phase 5 的工程完善内容：

- `src/features/ffn_activations.py`
- 更系统的注意力去偏与逐头 / 逐层选择机制
- 更完整的实验脚本与配置文件（如 `experiments/configs/*.yaml`）

因此，本报告当前已覆盖 **Phase 1 + Phase 2 + Phase 3 + Phase 4（方向 A）** 的主要实现与结果，剩余工程空缺集中在 FFN 特征实现、更系统的消融实验以及跨模型泛化验证。
...
- `python -s main.py phase4`：运行 Phase 4 注意力增强实验
- `python -s main.py phase4-attention`：只运行 Phase 4 注意力增强实验
...
3. **进阶特征仅完成方向 A**：当前仅实现了注意力模式分析，FFN 激活与其他高级结构特征仍待补充；
4. **模型与数据范围仍较有限**：当前结论基于单一模型 Qwen2-1.5B 与单一真假陈述数据集，外部泛化能力仍需验证；
5. **当前注意力特征仍受表面因素影响**：测试集差异最大的特征是 `sequence_length`，说明特征工程仍需更严格地控制句长与 token 数量偏差；
6. **统计显著性分析仍不充分**：虽然多随机种子和运行元数据已记录，但尚未补充更系统的显著性检验与跨设备重复实验统计。
...
- 推进 Phase 4：
  - 在当前 attention baseline 上继续做特征去偏、逐头 / 逐层选择与更细粒度消融；
  - 实现 FFN 特征并与当前 attention + hidden 基线比较；
- 扩展验证范围：
  - 增加更多随机种子与显著性检验；
  - 补充跨模型、跨数据集或跨领域验证；
- 完善正式报告：
...
本项目围绕“利用大语言模型内部状态进行幻觉检测”这一问题，基于 Qwen2-1.5B 与 True-False Dataset，已经完成了 Phase 1、Phase 2、Phase 3 与 Phase 4（方向 A）的实现与验证。Phase 1 搭建了完整的实验基础设施，包括配置管理、数据预处理、模型加载和测试体系；Phase 2 实现了 PPL 与 SAPLMA 两类基线方法；Phase 3 进一步回答了“哪一层更有用”“哪个 token 表示更合适”这两个关键分析问题；Phase 4 则初步验证了注意力统计特征在作为辅助信息时的增益与局限。
...
进一步地，Phase 3 显示真实性相关信号在中后层最强，而最后层并非最优读出位置；在最后层内部，`last` token 表示又明显优于 `mean` 与 `first` pooling。当前 Phase 4 则表明：原始注意力统计特征单独使用时几乎无法稳定完成真假检测，但在完成去污染、层选择、key token 锚点和 MLP 融合后，hidden+attention 已可将测试集 Accuracy 提升到 **0.8357**，明显高于原始 Phase 4 的 0.8067。这说明在该任务上，隐藏状态仍是主导性表征，而注意力模式在经过合理特征工程与非线性融合后，已经能够成为更有价值的辅助信号。下一阶段将围绕逐头注意力、FFN 特征实现以及更广泛泛化验证继续推进。
...
- `experiments/results/advanced/attention_ablation_logistic_layer17_last.json`
- `experiments/results/advanced/attention_ablation_logistic_layer17_last_improved.json`
- `experiments/results/advanced/attention_ablation_mlp_layer17_last_improved.json`
- `experiments/results/advanced/attention_ablation_logistic_layer17_last_improved_accuracy.png`
- `experiments/results/advanced/attention_ablation_logistic_layer17_last_improved_feature_deltas.png`
- `experiments/results/advanced/attention_ablation_mlp_layer17_last_improved_accuracy.png`
- `experiments/results/advanced/attention_ablation_mlp_layer17_last_improved_feature_deltas.png`
...
3. 已生成：注意力增强方法的消融对比图
4. 已生成：注意力特征差异图
5. 可补充：PPL 分数分布直方图（真/假对比）
6. 可补充：进阶方法与基线方法的统一总表
