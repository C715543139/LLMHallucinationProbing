...
Phase 4 复测进一步发现，在 Linux + RTX 3090 环境下，`eager + float16` 仍会产生 NaN，真正稳定的运行路径是 `eager + bfloat16`。在该设置下，全量 hidden-only baseline 达到 Accuracy 0.8082、Macro-F1 0.8081、AUROC 0.8897；在 600/150/150 子集上，`Hidden + top-16 head attention score` 融合取得最佳 Accuracy / F1（0.8867 / 0.8865），`Hidden + top-head + attention output` 三路融合取得最佳 AUROC（0.9403）。进一步的 A6 case analysis 显示，top-head attention 融合在 150 条测试子集上纠正了 4 个 hidden-only 错误、引入 1 个退化样本，净修正为 +3。这些结果说明 attention score 对 hidden state 具有稳定互补性，而 attention output 更适合作为排序补充信号。本报告据此总结 Phase 1 至 Phase 4 的实现、核心方法、实验结果与工程结论。
...
可以看到，浅层几乎接近随机水平，而从 layer 13 开始性能显著上升，layer 13 至 layer 20 形成稳定的高性能区间。相比之下，最后层（layer 27）的表现明显回落。这说明真实性相关信号在中后层已经充分形成，但在最终输出层中部分表征可能更偏向下一 token 预测而不是显式真假判别。当前方案按验证集 Accuracy 选择 layer 17，而不是根据测试集反向挑选层号；相较于 Phase 2 中归档的最后层逻辑回归基线（Accuracy 0.7496，AUROC 0.8265），选择 layer 17 后 Accuracy 提升约 0.0507，AUROC 提升约 0.0611。对应曲线图已保存为 `experiments/results/analysis/layer_accuracy_curve.png`。
...
Phase 4 以 Phase 3 已验证的最佳 hidden baseline 为起点，进一步考察注意力模块是否能提供互补信号。当前实现的主线是：先固定 `layer 17 + last token + logistic regression` 的 hidden-only 基线，再围绕陈述句的 `subject / relation / tail / last token` anchor 提取 attention score 特征、做去长度偏置、基于验证集筛选高价值 head，并将 attention output activation 作为另一类补充特征参与融合。
...
在 600/150/150 子集上，当前保留的核心消融结果如下：

| 方法 | 特征 | 维度 | Test Acc | Test F1 | Test AUROC | 核心观察 |
| ---- | ---- | ---- | -------- | ------- | ---------- | -------- |
| A0s | Hidden-only | 1536 | 0.8667 | 0.8661 | 0.9184 | 子集基线 |
| A2 | Debiased attn-score only | 1536 | 0.7733 | 0.7713 | 0.8285 | attention 单独已具判别力 |
| A4 | Attn-output only | 40 | 0.6800 | 0.6800 | 0.7493 | output 单独弱于 score |
| A5 | Hidden + debiased attn-score | 3072 | 0.8733 | 0.8729 | 0.9302 | 相比 A0s 有稳定提升 |
| A6 | Hidden + top-16 head attn | 1792 | 0.8867 | 0.8865 | 0.9330 | 最佳 Accuracy / F1 |
| A7 | Hidden + attn-output | 1576 | 0.8467 | 0.8463 | 0.9254 | AUROC 提升但 Acc 回落 |
| A8 | Hidden + top-head + output | 1832 | 0.8800 | 0.8798 | 0.9403 | 最佳 AUROC |
| A9 | Gated Fusion | - | 0.8667 | 0.8661 | 0.9193 | 无净纠错收益 |

从这些结果可以得到三点直接结论。第一，attention score 单独使用时已经明显高于随机，说明在 NaN 消失后，注意力分数本身就含有真实性相关信号。第二，最有效的增强方式不是直接拼接全部 attention 统计，而是先做 validation-based head selection，再将 top-head score 与 hidden state 融合；A6 相比 A0s 的 Accuracy 提升了 2.00 个百分点。第三，attention output 更像排序补充信号而不是独立决策器：A7 的 AUROC 高于 A0s，但准确率下降；A8 在加入 output 后拿到最高 AUROC，却没有超过 A6 的准确率。
...
当前 head selection 的验证集 AUROC 分布落在 0.6069 至 0.6776 之间，最强单 head 为 `L16-H05`。此外，A9 的错误修正矩阵显示净纠错数为 0，说明当前简单 gated fusion 还不足以稳定替代直接特征拼接。

#### 5.5.3 A6 错误修正与案例可视化

为了避免只依赖总体指标判断 Phase 4 是否有效，项目进一步补充了 `Hidden-only` 与 A6 `Hidden + top-16 head attention` 的逐样本对比，结果保存为 `experiments/results/phase4/a6_case_analysis.csv` 与 `experiments/results/phase4/a6_correction_matrix.json`。在 150 条测试子集样本上，A6 的修正矩阵如下：
...
该矩阵说明，A6 在保留大部分 hidden-only 正确样本的同时，纠正了 4 个 hidden-only 错误样本，但也引入了 1 个退化样本，净修正为 +3。典型改进案例包括 `The Earth has one moon.`、`Carl Elsener lived in Switzerland.` 与 `Reliance Industries engages in hydrocarbon exploration...`。这些案例说明 top-head attention score 的收益不仅体现在 A6 的总体 Accuracy 提升上，也能在部分具体样本中改变错误决策。

项目还基于最强 head `L16-H05` 生成了定性 attention 可视化，保存于 `experiments/results/phase4/case_viz/`，包括 true/false correct cases、hard/failure cases 与 improvement cases。需要强调的是，attention case 图用于解释和展示模型在关键 token 上的注意力模式，不能单独作为因果证明；最终有效性判断仍以 A0-A9 消融结果和 A6 修正矩阵为准。

![A6 improvement attention case](../experiments/results/phase4/case_viz/improvement_case_l16_h5_1.png)
...
Phase 2 说明，在固定最后层 + `last` token 的前提下，MLP 相比逻辑回归能够进一步提升性能；但 Phase 3 又表明，单纯更换表示层带来的收益并不小于提升分类器复杂度。具体地，`layer 17 + last + logistic` 的 Accuracy 为 0.8003，高于 `layer 27 + last + mlp` 的 0.7771。这说明后续工作不应只关注“换更强的分类器”，还应联合搜索层深度、token 表示与分类器类型。另一方面，重跑后的结果差异大多保持在可接受范围内，也说明当前结论主要反映表示质量差异，而不是实验流程不稳定。

### 6.6 为什么 attention score 融合能够继续提升

Phase 4 的结果表明，attention 特征并非简单的“附属解释变量”，而是真正可以补充 hidden baseline 的结构化信号。其原因在于：hidden state 更偏向整句压缩表示，而 attention score 显式保留了 `last token` 与 `subject / relation / tail` 之间的对齐关系；在对长度因素做残差化处理、并通过验证集筛选出高价值 head 之后，这类结构信号更容易与 hidden 表征形成互补。A6 优于 A5，也说明“先筛 head 再融合”比“保留全部 score 统计再拼接”更有效，因为它减少了噪声维度并保留了最具判别力的注意力模式。A6 的逐样本修正矩阵进一步表明，这种互补性不是只反映在总体指标上：在 150 条测试子集中，A6 对 hidden-only 的净修正为 +3。

### 6.7 Phase 4 暴露的工程边界

Phase 4 同时揭示了三个重要边界。第一，Linux 平台本身不足以自动解决数值问题；真正决定实验是否可用的是 `dtype + attention implementation` 的组合，当前可复现实验必须走 `eager + bfloat16`。第二，attention output 虽然能改善 AUROC，但并未像 top-head score 那样稳定提升准确率，这提示 attention 模块写回 residual stream 的激活更适合作为排序补充而非主决策特征。第三，A6 的增益目前来自 600/150/150 子集，且净修正为 +3，说明它是有价值但仍需扩大验证的改进，而不是已经完成全量泛化证明。再加上 gated fusion 在当前设置下没有带来净纠错，说明后续改进应优先集中在更稳健的特征筛选、路由策略与全量数据验证，而不是盲目增加融合复杂度。
...
当前剩余工作已经不再是“Phase 4 是否能实现”，而主要集中在以下收尾与扩展方向：

1. 将 A5-A9 等融合实验进一步扩展到全量数据集，验证子集结论是否稳定迁移；
2. 在 Qwen2-1.5B 之外补充跨模型验证，检查 attention-guided 特征的泛化性；
3. 补充更系统的显著性检验、校准分析与跨设备重复实验统计；
4. 针对 gated fusion 与错误修正策略继续探索更有效的路由机制；
...
1. **A5-A9 融合结果当前主要基于子集实验**：虽然结果趋势清晰，但全量数据上的最终收益仍需系统验证；
2. **模型与数据范围仍较有限**：当前结论主要基于单一模型 Qwen2-1.5B 与单一真假陈述数据集，外部泛化能力仍需验证；
3. **anchor 抽取仍以规则法为主**：面对更复杂句法结构时，subject / relation / tail 对齐稳定性仍可能下降；
4. **统计显著性分析仍不充分**：虽然多随机种子和运行元数据已记录，但尚未补充更系统的显著性检验与置信区间分析；
5. **复杂融合策略尚未形成稳定优势**：当前 gated fusion 没有带来净纠错；A6 虽然获得净修正，但仍需要全量数据与更多模型验证其稳定性。

### 8.2 后续工作计划

后续工作更适合沿以下方向推进：

- 将 top-head attention 融合与三路融合扩展到全量数据集，验证 Accuracy 与 AUROC 提升是否保持；
- 对更多模型与领域数据开展复现实验，检查“中后层 + top-head attention”结论的迁移性；
...
当前结果表明，PPL 虽然具备一定排序能力，但最终分类性能明显不足；相比之下，基于隐藏状态的 SAPLMA 能够更有效地利用模型内部表征。Phase 3 显示真实性信号在中后层更强，`layer 17 + last + logistic` 明显优于固定最后层的基线。Phase 4 则进一步表明：在解决 NaN 问题后，attention score 已具备独立判别力；通过 validation-based head selection 后与 hidden state 融合，可以把子集 Accuracy 从 0.8667 提升到 0.8867，并在 A6 case analysis 中取得 +3 的净错误修正；而加入 attention output 后可以把 AUROC 提升到 0.9403。

更重要的是，Phase 4 的工程复测澄清了一个关键事实：Linux 平台本身并不会自动修复 attention 路径上的 NaN，真正有效的稳定方案是 `eager + bfloat16`。因此，本项目当前的核心结论不仅包括“内部状态确实知道陈述真假”，也包括“这些信号在 hidden state 与 attention score 之间具有可组合性，但 attention output 与复杂 gated fusion 的收益边界更窄”。后续若继续扩展，本项目最值得优先推进的方向将是全量数据验证、跨模型泛化与更稳健的融合策略。
...
- `experiments/results/phase4/a6_case_analysis.csv`
...
- `experiments/results/report_assets_manifest.md`
...
5. 已生成：A6 correction matrix、A6 逐样本对比表与 attention case 可视化
