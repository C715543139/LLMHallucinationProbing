...
当前已完成 Phase 1 与 Phase 2。Phase 1 主要完成实验环境搭建、数据预处理、模型加载与测试体系构建；Phase 2 主要实现 PPL 基线与 SAPLMA 基线，并完成真实代码验证。基于最终结果文件，SAPLMA 方法明显优于 PPL 方法：PPL 在测试集上的 Accuracy 为 0.5293、Macro-F1 为 0.4180、AUROC 为 0.6784，而在修改后的复现设置下重跑得到的 SAPLMA-MLP 在测试集上的平均 Accuracy 为 0.7697、Macro-F1 为 0.7696、AUROC 为 0.8676。该结果说明，相比于受句长和表面流畅性影响较大的概率分数，模型隐藏状态中编码了更适合真假判别的内部信号。
...
本项目采用第二类思路，以论文 _The Internal State of an LLM Knows When It's Lying_ 为主要参考，围绕 True-False Dataset 中的陈述句开展实验。项目首先实现两类基线方法：其一是基于序列困惑度的 PPL 方法，其二是基于隐藏状态分类的 SAPLMA 方法。随后，项目计划进一步分析不同层深度、不同 token 位置表示对真实性检测的影响，并探索注意力或 FFN 激活等更丰富的内部特征。
...
逻辑回归已经能达到较好的结果，说明隐藏状态中存在明显的线性可分性；而 MLP 进一步提升表现，则说明这种可分性并非完全线性，仍存在一定的高阶交互模式。另一方面，在显式固定 float16、随机种子与确定性运行选项后重新实验，MLP 相对于前次结果的差异总体控制在 ±0.01 范围内，且方法排序保持不变。这说明当前结果在结论层面是稳定的，而较大的标准差主要来自非线性分类器训练本身的随机波动，而不是实验流程失控。后续在层分析中，可以继续观察：
...
从当前实验结果看，SAPLMA-MLP 仍是已实现方法中的最佳基线。并且在修改后的复现设置下重跑后，结果与此前运行相比差异控制在 ±0.01 内，说明结论具有可接受的稳定性。这一结果为后续 Phase 3 的层深度分析与 token 位置分析，以及 Phase 4 的注意力 / FFN 特征探索，提供了坚实基础。
