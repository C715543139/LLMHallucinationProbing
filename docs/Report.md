...
当前 top-16 head selection 的验证集 AUROC 分布落在 0.5875 至 0.6527 之间，最强单 head 为 `L15-H06`。此外，A9 的错误修正矩阵显示净纠错数为 +3，但总体指标仍低于 A2，说明当前简单 gated fusion 可以修正少量样本，却不足以替代 debiased attention-score only 方案。
...
![Attention-guided improvement case](../experiments/results/phase4/case_viz/improvement_case_l15_h6_1.png)
...
当前主体实验、结果资产与英文 ACL 风格报告项目已经形成，后续工作主要集中在以下扩展与提交前检查方向：
...
5. 提交前检查 `report/main.tex` 的作者信息、图表排版与案例展示是否符合最终课程要求。
...
5. **复杂融合策略尚未形成稳定优势**：当前 A6 top-head 融合的聚合 Accuracy 低于 hidden-only baseline，A9 gated fusion 虽有 +3 净修正但总体指标低于 A2，说明融合与路由策略仍需重新设计。
...
- 提交前复核正式论文中的数值稳定性诊断、错误案例与可解释图表呈现；
