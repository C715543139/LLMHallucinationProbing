...
| Phase 5：报告与答辩准备        | 6.3 – 6.9      | 进行中 | 结果资产已补齐 |
...
| SAPLMA (Logistic) | 0.7496 ± 0.00 | 0.7496 ± 0.00 | 0.8265 ± 0.00 | 3 seeds 均值 |
| SAPLMA (MLP)      | 0.7771 ± 0.01 | 0.7769 ± 0.01 | 0.8770 ± 0.00 | 重跑最佳结果 |
...
| ------- | ------------- | ------------- | ---------- |
| last    | 0.7496        | 0.7496        | 0.8265     |
| mean    | 0.7052        | 0.7052        | 0.7636     |
| first   | 0.4200        | 0.4197        | 0.3794     |

**初步结论**：
- 真实性信号在中后层（layer 13–20）达到最强，最后层反而回落，说明最接近输出的位置不一定最适合真假判别
- `last > mean >> first`：自回归模型中最后一个有效 token 是最优的整句读出位置
- 表示选择的重要性不低于分类器复杂度：`layer 17 + last + logistic`（Acc 0.8003）优于 `layer 27 + last + mlp`（Acc 0.7771）
...
4. **A6 具备样本级净修正**：在 150 条测试子集上，A6 相比 hidden-only 纠正 4 个错误、引入 1 个退化样本，净修正 +3
5. **复杂路由不一定优于直接融合**：当前的 simple gated fusion（A9）没有带来净纠错收益
6. **数值稳定性是前提**：Linux + RTX 3090 上 `eager + float16` 仍产生 NaN，`eager + bfloat16` 是唯一可复现的稳定路径
...
| 报告图表资产 | `experiments/results/phase4/figures/`, `case_viz/`, `a6_case_analysis.csv` | 完成 |
...
| P2   | SAPLMA-MLP (layer 27, last)   | 0.7771   | 0.7769  | 0.8770     |
| P3   | Layer 17 + last + LR          | 0.8003   | 0.8001  | 0.8876     |
...
| 任务                       | 状态/优先级 | 预计时间   | 说明                                     |
| -------------------------- | ------ | ---------- | ---------------------------------------- |
| 文档数字口径统一           | 高     | 5.31–6.1   | 以 `scripts/show_results.py` 和归档 JSON/CSV 为最终真源 |
| 实验报告撰写（PDF）        | 高     | 6.1–6.7    | 整合 Phase 1–4 结果，完成正式实验报告     |
| 图表与可视化完善           | 已完成 | 5.31       | 已补 PPL 分布、Phase 4 对比图、A6 修正矩阵和 attention case |
| 代码整理与 README 完善     | 中     | 6.1–6.7    | 补充项目 README、运行说明、复现指南       |
| 答辩材料准备               | 中     | 6.6–6.8    | 准备 PPT 或展示材料，梳理关键结论与创新点 |
| 融合实验全量数据验证       | 可选   | 后续扩展   | 将 A5–A8 等融合实验扩展到全量 6309 样本，确认子集结论的迁移性 |
