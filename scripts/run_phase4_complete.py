"""
Phase 4 完整评估：SDPA 取 hidden + eager(batch=1) 取 attention + 全部 A0-A9 消融。

关键修复：
  - Hidden states 在 SDPA 模式下提取（支持 batch padding）
  - Attention 特征在 eager 模式下逐样本提取（batch=1, 无 padding）
  - 两者互不干扰，避免 eager+batch+padding → NaN

用法:
    conda activate llm_hallucination
    .\.venv\Scripts\activate.ps1
    python -s scripts/run_phase4_complete.py
"""
...
SUBSET_SIZES = {"train": 600, "val": 150, "test": 150}  # 子集加速
...
# ===================================================================
# Phase 1: SDPA 模式 — 提取 hidden states
# ===================================================================
print("=" * 60)
print("  Phase 4 Complete Evaluation (SDPA→hidden / eager→attention)")
print("=" * 60)
print_device_info()

t_total = time.time()

print("\n[Phase 1] Loading model (SDPA) + extracting hidden states...")
t0 = time.time()
model, tokenizer = load_model_fp16(model_path=str(config.paths.models_cache / "Qwen2-1.5B"))
print(f"  Model loaded ({getattr(model.config,'attn_implementation','default')}) in {time.time()-t0:.0f}s")

train_ds, val_ds, test_ds = load_processed_data()
print(f"  Data: {train_ds.summary().split(chr(10))[0]}")

# ---- 提取 FULL hidden states (SDPA, batch=8, 支持padding) ----
...
for name, arr in [("train", X_h_train), ("val", X_h_val), ("test", X_h_test)]:
    nc = np.isnan(arr).sum()
    if nc > 0:
        logger.warning(f"  {name}: {nc} NaN → 0")
...
# ---- Hidden Baseline (SDPA hidden) ----
print("\n  Hidden Baseline (L17 last, LR)...")
...
# ===================================================================
# Phase 2: EAGER 模式 — 提取 attention 特征 (逐样本, batch=1)
# ===================================================================
print("\n[Phase 2] Switching to EAGER attention for attention feature extraction...")
...
# ---- 提取 attention scores (eager, batch_size=1) ----
print("  Extracting attention scores (eager, batch=1)...")
t0 = time.time()
...
print(f"  Attention scores: {X_as_train.shape} ({time.time()-t0:.0f}s)")

# ---- 提取 attention outputs (eager, batch_size=1) ----
print("  Extracting attention outputs (eager, batch=1)...")
t0 = time.time()
...
print(f"  Attention outputs: {X_ao_train.shape} ({time.time()-t0:.0f}s)")

# ---- 切回 SDPA (后续不再需要模型前向传播) ----
print("  Switching back to SDPA...")
...
# ===================================================================
# Phase 3: 长度残差化 + Head Selection
# ===================================================================
print("\n[Phase 3] Length residualization + Head selection...")

# 使用 attention sink 作为长度代理
...
print(f"  Residual: corr={resid_meta['correlation_before']:.4f}→{resid_meta['correlation_after']:.4f}")
...
# Phase 4: A0-A9 消融实验
# ===================================================================
print("\n[Phase 4] Running A0-A9 ablation experiments...")
all_results = []
...
# A0: hidden-only (FULL dataset)
run("A0", "Hidden-only (L17 last, LR, FULL)", X_h_train, X_h_val, X_h_test,
    y_train_full, y_val_full, y_test_full, "Full dataset (5047 samples)")
...
error_rows = build_error_analysis(
    test_sub.statements, y_sub_test, h_test_p, gf["fused_probs"], h_preds, f_preds)
...
with open(OUTPUT_DIR / "phase4_error_analysis.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=error_rows[0].keys() if error_rows else [])
    writer.writeheader()
